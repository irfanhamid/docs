...
déclencher ainsi qu'un objet protégé de type synchr\^oniseur sur
...
que l'on a appellé synchr\^oniseur et auquel nous avons déjà fait
...
\caption{Synchr\^oniseur d'un thread sporadique nommé \texttt{Thr1} du type \texttt{TType}}
...
synchr\^oniseur décrit ci-dessus est généré, dont la priorité de
...
\begin{description}
\item[Denotationelle :]{Chaque expression ou instruction dans le langage de programmation est traduit envers une domaine math\'ematique, ce domaine d\'ecrit 

Each statement or instruction in the language
...
\item[Operationelle :]{An operational semantics provides the meaning of
  programs via a state transition formalism~\cite{plotkin-sos}. Each
  transition corresponding to one ``step'' of the program being
  modeled;}
\item[Axiomatique :]{The semantics of a program are given via assertions
