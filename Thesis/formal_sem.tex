...
represented by $\iota$; or an activity $a$, also called an
\emph{active context}. Hence, the execution context, $c$, is given by
the following:
\begin{displaymath} 
\centering
...
\begin{tabular}{ll}
  \emph{Active contexts of sporadic tasks}: & $S$, $S \rset E$, $S \rsvt D$, $S \rget E$, $S \rgvt D$\\ & \\
  \emph{Active contexts of periodic tasks}: & $P$, $P \rset E$, $P \rsvt D$, $P \rget E$\\ & \\
  \emph{Active contexts of interrupts}: & $U$, $U \rset E$, $U \rsvt D$\\
\end{tabular}
\\

%% \noindent\emph{Active contexts of sporadic tasks}:
%%     \hspace*{2mm} $S$,  
%% 	\  $S \rset E$, 
%% 	\ $S \rsvt D$, 
%% 	\ $S \rget E$, 
...
\noindent where the form $\alpha \rx \pi$ corresponds to the context
of a protected object $\pi$ executing a call $x$ issued by activity
$\alpha$. The {\scshape priority} function is extended to active
contexts in conformance with the PCP:
\begin{equation}
\label{eq:pcp_rule}
...
associated to task $T$. The update of a field in a record is
performed as:
\begin{center}
$D'=
  \langle
...
\end{center}
\noindent
where record $D'$ is equal to record $D$ except for the field
...
\hline
\end{tabular}
\caption{Fields present in state records of Ravenscar Computational
  Units.}
\label{tab:fields}
\end{table}
...
ordered list of queues of the form $R= r_{p_1} \smcirc \ldots \smcirc
r_{p_n}$ where each $r_{p_i}$ is a subqueue satisfying:
\begin{eqnarray}
...
Eq.~(\ref{samepriority}) enforces that all active context members of
the same subqueue have the same priority, and
Eq.~(\ref{priorityorder}) enforces that subqueues are ordered
according to their priorities. Two ways of inserting an active context
into a ready queue are defined, priority head insertion and priority
tail insertion:\\

\noindent
$Let\ p_k = \text{\scshape priority}(a) $\\
\\
...
definition stipulates that tasks may be inserted into the ready queue
in two different ways:

\begin{enumerate}
\item{A task that is taken from the blocked set of tasks and put in
  the ready queue is inserted at the \emph{tail} of the ready subqueue
  of its priority;}
...
depend on the current state of the code of its activity, which is
given by its state record's \SFT{Beh} field. The steps that may be
performed by the scheduler or the idle task are given in
Table~\ref{legal_idle_sched}.
...
Here \emph{Antecedents} are the conditions which need to hold in order
for the \emph{Consequent}---the numerator---part to be applied. The
\emph{Antecedents} depend on the current state of the system. The
\emph{Consequent} denotes the transition taken and the
action---$act$---performed. The action $act$ represents the smallest
possible uninterruptible instruction that can be executed by the
system. It is an indivisible unit in that interrupts (from external
devices) and timers will either be fired before or after such an
instruction, never during it. Complex instructions like \kw{delay
...
\emph{aging} operator, defined formally as:
\begin{displaymath}
  \big\langle c,R,B,\SFT{ns},\SFT{t} \big\rangle 
\sqplus \delta (act) = 
...
The above equations state that if the currently execution context is
either the scheduler or the idle task then the aging operator has no
effect on it. However, for an active execution context the aging
operator adds the $\delta(act)$ amount of time to both the elapsed
time (\SFT{Et}) and processing time (\SFT{Pt}) fields of the record of
its activity. On the other hand, for all tasks in the ready queue $R$,
the aging operator only adds the $\delta(action)$ amount of time to
the elapsed time field (they are not budgeted for this time). The
following subsections give the transition rules for the system.
...
task is ready to run. As a result, the \emph{age} of the system
advances by an amount $\delta(idling)$, and $\iota$ stays the active
context.
...
carrying out a sequential computation and \emph{CMPO} represents a
protected object's code carrying out a sequential computation. The
behavior (\SFT{Beh}) must in both cases have a \texttt{comp}
...
The \emph{CMPT} transition modifies the state of the system by aging
...
remaining steps of its {\scshape prog}.
...
The \emph{CMPO} transition modifies the state of the system by aging
the system with the appropriate time interval. The ready queue and
blocked set remain the same, the active context remains the same as
well as the task has not yet returned from the protected object and
has not been interrupted.

\subsubsection{Capturing dispatch time of sporadic tasks}
The \emph{SYCT} transition mirrors the execution of the first
instruction of an \texttt{Await\_Event} entry of a synchroniser. This
...
blocked on this synchronizer's \texttt{Await\_Event} entry. The
execution time is budgeted to the task's processing time \SFT{Pt} and
the scheduler takes over to evaluate the barrier of this synchronizer.
...
\emph{RET4} shows a synchronizer returning from an
\texttt{Await\_Event} entry. The task in whose context the execution
was taking place is preempted, it is placed at the head of its ready
queue, and the scheduler takes over.
...
Rules \emph{OBCL} and \emph{CBCL} represent a sporadic task issuing an
\texttt{Await\_Event} call to a synchronizer. Rule \emph{OBCL}
reflects the case where the barrier is open and the call is
immediately executed. Rule \emph{CBCL} mirrors the case where the
barrier is closed, thus the call remains blocked in the synchroniser
...
as the \emph{RET$_i$} transitions. Others occur when an active context
executes a \kw{delay until} instruction, and when the scheduler is
\emph{scheduled} to execute. This time is denoted by the $ns$ variable
in the system configuration and is calculated just before the
scheduler cedes control to one of the tasks or
interrupts. \emph{NS-IDLE} represents the scheduler preempting the
idle task because the scheduler's launch time has arrived. The system
time must be equal to $ns$ and the system configuration must state
that the execution context is the idle task $\iota$. As a consequence,
the idle task is replaced by the scheduler on the CPU and the system
is aged by the amount of time required for this action, represented by
$\delta(is)$.
...
context must not be the idle task or the scheduler and the current
time should be the scheduler's dispatch time $ns$. The consequence is
that the executing active context is preempted and the scheduler's
initial state is put on the processor. The system is aged by the
amount of time required for this preemption, represented by
$\delta(as)$.
...
two different states $\sigma$ and $\sigma_s$. The antecedent states
...
context must be the scheduler's final state $\sigma_s$ and the ready
...
the scheduler's final state $\sigma_s$ and the ready queue must be
...
service a higher priority interrupt. The preconditions being that an
interrupt must currently be executing. The consequence is that a new
interrupt $U'$ is present in the list of interrupts, the executing
interrupt $U$ has been placed at the head of its ready queue, the
scheduler is executing in its initial state and the system has been
aged by $\delta(ius)$.
...
\formatreg {
  \regle { 
    ...
    \big\langle c,R,B,\SFT{ns},\SFT{t}\big\rangle
    \Faitv{ius}
    U'\smcirc\textit{\small IL}\intrupt
    \big\langle\sigma,U\smodot R,B,\SFT{ns},\SFT{t}\big\rangle\sqplus\delta(ius)
  }
  \textit{\footnotesize I-US}
}

\emph{I-IS} represents the scheduler preempting the idle task to
handle an interrupt. The execution context must be the idle task for
this transition to occur. The consequence is that the scheduler is now
executing in its initial state and the system is aged by
$\delta(iis)$.
...
precondition is that the scheduler be executing.. The consequence is
that the scheduler is restarted and the system is aged by
$\delta(iss)$.

\formatreg {
  \regle {
      c=\sigma \vee c=\sigma_s\\
  } {
...
antecedent states that the scheduler be running and there be at least
one interrupt in the interrupt list. The consequent is that the
highest priority interrupt is taken from the head of the interrupt
list and is added at the head of the ready queue. The system is aged
by $\delta(iih)$.
...
and containments. The dynamic semantics gives the evolution of a
...
understand formalism for giving meaning to programs. This allows the
...
formal meaning of a subset of AADL constructs. The formalization of
this semantics led to the discovery of the small discrepancy in the
Ravenscar Profile which can adversely impact response-critical
sporadic tasks and a solution for which was presented in
Sec.~\ref{sec:response_crit_st}.
...
finally allow the conversion of the specification into an automaton or
labeled transition system. Such an implementation of the semantics
would be useful from two distinct points-of-view:
...
\item[Simulation:]{Any simulator for a Ravenscar system, or for any
  ...
  to disambiguate for the implementer the meaning of every single
  aspect. A case in point is the LOTOS verification language which has
  a formal semantics~\cite{turner@pstv87}. A counterpoint is the C
  programming language, which does not have formal semantics,
  resulting in compiler dependent behavior;}
